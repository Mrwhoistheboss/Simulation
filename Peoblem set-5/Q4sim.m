function [N,T]=Q4sim(lambdaV,mu,endtime)
...
if length(lambdaV)>1
    lambdaStep=1;
    event(1)=lambdaV(lambdaStep);   %exprnd(1/lambda); %time for next 
    lambdaStep=lambdaStep+1;
else
    event(1)=exprnd(1/lambdaV); %time for next
end
%%%%%%%%%%%%%%%%%%%%
...
while t<endtime 
    ...
    if nextevent==1
        %%%%%%%%%%%%%%%%%%%
        if length(lambdaV)>1
            event(1)=lambdaV(lambdaStep)+t; %exprnd(1/lambda)+t; %modified
            lambdaStep=lambdaStep+1;
        else
            event(1)=exprnd(1/lambdaV)+t; %modified
        end
        %%%%%%%%%%%%%%%%%%%%
