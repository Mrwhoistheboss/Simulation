function [N,T,R]=Q2sim(lambda,mu,endtime, queSize)
...
%%%%%%%%%%%%%
actulQueLength=queSize; %modified
rejectedCustomer=0; %modified
%%%%%%%%%%%%%%
while t<endtime
...
%         if currcustomers< actulQueLength+1
%             nbrarrived=nbrarrived+1; %one more customer has arrived to the system through the simulations
%             arrivedtime(nbrarrived)=t; %the new customer arrived at time t
%             if currcustomers==1
%                 event(2)=exprnd(1/mu)+t;
%             end
%         else
%            rejectedCustomer=rejectedCustomer+1;
%         end
        %%%%%%%%%%%%%%%%%%%%
    elseif nextevent==2
        %%%%%%%%%%%%%%%%%%%%%%
        if currcustomers<= queSize
            currcustomers=currcustomers-1;
            if actulQueLength>0
                actulQueLength=actulQueLength-1;
            end
           ...
           rejectedCustomer=rejectedCustomer+1;
           currcustomers=currcustomers-(currcustomers-queSize);
           if actulQueLength>0
                actulQueLength=actulQueLength-1;
            end
        end
...
%         nbrdeparted=nbrdeparted+1; %one more customer has departed%from the system through the%simulations
%         T(nbrdeparted)=timeinsystem;
%         if currcustomers>0 && currcustomers<actulQueLength
%             event(2)=exprnd(1/mu)+t;
%         else
%             event(2)=inf;
%         end
        ...
        %%%%%%%%%%%%%%%%%
        R(nbrmeasurements)=rejectedCustomer/nbrmeasurements;%modified
        %%%%%%%%%%%%%%%%%
